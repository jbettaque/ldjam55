...
function game.summoning.update(dt)
...
function game.summoning.draw()
    if game.state.summoning.isSummoning then
        ...
        love.graphics.printf(game.conf.ui.summoning.menuTitle, x1 + menuBorder, y1 + menuBorder + titleMargin, tableWidth, "center")
...
function game.summoning.keypressed(key)
    ...
    if game.state.summoning.isSummoning and number and number > 0 and number <= table.getn(game.state.summoning.types) then
        game.state.summoning.isSummoning = false
